...
export default function PortalShellLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (session.user.role !== "portal_client") { router.replace("/"); return; }
      const { data: aal } = await supabase.auth.mfa.getAuthenticatorAssuranceLevel();
      if (aal?.currentLevel !== "aal2") { router.replace("/portal/login"); return; } // MFA gate (UX; the DB enforces regardless)

      // suspended mappings keep the session but the DB returns no org data
      const { data: self } = await supabase.from("portal_users").select("status").maybeSingle();
      if (self?.status === "suspended") { if (!cancelled) setState("suspended"); return; }

      // RLS returns exactly the client's own org row — no app-side filter
      const { data: orgRow } = await supabase.from("crm_organisations").select("display_id, name").maybeSingle();
      if (!cancelled) { setOrg(orgRow as PortalOrg | null); setState("ready"); }
    })();
