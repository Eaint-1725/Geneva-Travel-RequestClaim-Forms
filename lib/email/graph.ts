...
// --- Multi-attachment send (Excel + uploaded files) -------------------------------------
//
// sendGraphEmail above is untouched and stays the single-attachment code path. Travel Request's
// Approval Attachments need to send the request Excel plus one or more uploaded files, and
// Graph's one-shot sendMail only reliably takes attachments under ~3MB inline as base64 in the
// same request -- anything larger needs its own upload session, which requires an existing
// message. So this creates a draft message, attaches each file (small ones in one call, large
// ones via a chunked upload session), then sends the draft. New exports only -- nothing above
// changes.
