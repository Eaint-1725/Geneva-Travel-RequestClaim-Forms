...
export interface TravelRequestWorkbookResult {
  buffer: Buffer;
  fileName: string;
...
/** Single source of truth for the Travel Request .xlsx -- used by both the download route and the HR email attachment. */
export async function buildTravelRequestWorkbook(form: TravelRequestForm): Promise<TravelRequestWorkbookResult> {
  ...
  for (const trip of form.trips) {
    ...
    // does not touch calc.ts or the on-screen totals.
    ...
    trip.rows.forEach((r, i) => {
      writeRow(row, r, i === 0, excelPerDiemUsd[i], rowCalcs[i].amountMmk);
      row += 1;
    });
  ...
  const fileName = `Travel Request - ${safeName} - ${form.header.month || "draft"}.xlsx`;
