...
import { formatAutoRateCaption, type UnRate } from "@/lib/travel/un-rates";
...
export default function ClaimRowFields({
  row,
  rowIndex,
  rowLabel,
  onChange,
  ...
  /** The UN rate resolved for this row's Date (§4), or null if no Date yet / none on file. */
  resolvedRate: UnRate | null;
  perDiemUsd: number;
  amountMmk: number;
  /** Header's Submission Date -- the row Date's ceiling (see validateRow). Empty until chosen. */
  submissionDate: string;
}) {
  const set = <K extends keyof Row>(field: K, value: Row[K]) => onChange({ ...row, [field]: value });
  const tid = (suffix: string) => `travel-claim-row-${rowIndex}-${suffix}`;

  const rateCaption = !row.date
    ? "Set a date to derive the rate"
    : resolvedRate
      ? formatAutoRateCaption(resolvedRate)
      : "No UN rate on file for this date";

  // Gentle nudge instead of a confusing validation error while there's nothing to check the row
  // Date against yet (see validateRow) -- suppressed once an error already shows on this field,
  // so the two never stack.
  const dateHint = !submissionDate && !errors.date ? "Select the submission date first" : undefined;

  return (
    <div className="mb-2" data-testid={`travel-claim-row-${rowIndex}`}>
      <p className="mb-1 text-[11px] uppercase tracking-wide text-gray-500">{rowLabel}</p>
      <div className="flex flex-col gap-3 md:grid md:grid-cols-2 md:gap-x-3 md:gap-y-3 lg:flex lg:flex-row lg:flex-wrap lg:gap-2">
        <Field label="Date" error={errors.date} hint={dateHint} hintTestId={tid("date-hint")} width="w-full lg:w-36">
        ...
        <Field label="No of days" error={errors.noOfDays} width="w-full lg:w-20">
        ...
        <Field label="Deductions" error={errors.deduction} width="w-full lg:w-56">
        ...
        <div className="flex w-full flex-col lg:w-44">
          <Field label="Exchange rate (MMK per USD)" error={errors.exchangeRate}>
            <input
              type="text"
              readOnly
              className={`${inputCls} w-full`}
              value={row.date ? (resolvedRate ? resolvedRate.rate.toLocaleString("en-US") : "Unavailable") : ""}
              placeholder="—"
              data-testid={tid("exchange-rate")}
            />
          </Field>
          <p className="mt-0.5 text-[11px] text-gray-500" data-testid={tid("exchange-rate-caption")}>
            {rateCaption}
          </p>
        </div>
        ...
        <Field label="IPO Number (optional)" width="w-full lg:w-32">
        ...
        <Field label="Remark (optional)" width="w-full lg:w-40">
